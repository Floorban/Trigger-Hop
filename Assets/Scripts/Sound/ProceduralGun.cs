...
public class ProceduralGun : ScriptableObject
{
    ...
    public void PlaySound(Vector3 playPosition)
    {
        if (!GunSound.IsNull)
        {
            i_GunSound = SceneController.instance.gameAudio.CreateInstance(GunSound, playPosition);
            ...
            i_GunSound.setParameterByName("Barrel Length", barrelLength);
            i_GunSound.setParameterByName("Barrel Diameter", barrelDiameter);
